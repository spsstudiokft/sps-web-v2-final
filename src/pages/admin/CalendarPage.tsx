...
export default function CalendarPage() {
  ...
  return <div className="flex h-full min-h-0 flex-col overflow-hidden bg-background text-text">
    ...
    <div ref={calendarViewportRef} className="min-h-0 flex-1 touch-pan-x touch-pan-y overflow-auto overscroll-contain [scrollbar-gutter:stable]">
      <div className="min-w-[974px]">
        ...
        <div className="sticky top-[57px] z-20 grid min-h-11 border-b border-border bg-surface/90 shadow-sm backdrop-blur-xl" style={{gridTemplateColumns:"64px repeat(7, minmax(130px, 1fr))"}}>
          <div className="sticky left-0 z-30 flex items-start justify-end border-r border-border bg-surface/95 px-2 py-3 text-[9px] font-bold uppercase tracking-wider text-muted-text">Egész nap</div>
    ...
    {active && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/55 p-3 backdrop-blur-sm sm:p-4" onMouseDown={e=>e.target===e.currentTarget&&setActive(null)}><form onSubmit={save} className="flex max-h-[94dvh] w-full max-w-xl flex-col overflow-hidden rounded-2xl border border-border bg-surface shadow-2xl">
      <div className="flex shrink-0 items-center justify-between border-b border-border px-5 py-4"><div><h2 className="font-bold">{active.id?"Esemény részletei":"Új esemény"}</h2>{active.id&&<p className="mt-0.5 text-xs text-muted-text">{active.owner_name}</p>}</div><button type="button" className="rounded-lg p-2 hover:bg-background" onClick={()=>setActive(null)}><X size={18}/></button></div>
      <div className="min-h-0 flex-1 space-y-4 overflow-y-auto p-5 [scrollbar-gutter:stable]">
        ...
        {(active.event_type==="project"||active.event_type==="portfolio")&&<label className="block"><span className="mb-1.5 block text-xs font-semibold text-muted-text">{active.event_type==="project"?"Kapcsolt projekt neve":"Kapcsolt portfóliógaléria neve"}</span><input required disabled={!active.can_edit} className="w-full rounded-xl border border-border bg-background px-3.5 py-3 outline-none focus:border-primary disabled:opacity-70" value={active.resource_name||""} onChange={e=>setActive({...active,resource_name:e.target.value})} placeholder={active.event_type==="project"?"Add meg a projekt külön nevét":"Add meg a galéria külön nevét"}/><span className="mt-1.5 block text-[10px] text-muted-text">Ez a név a kapcsolt elemhez tartozik, és független a naptári esemény nevétől.</span></label>}
        <div><span className="mb-2 flex items-center gap-2 text-xs font-semibold text-muted-text"><UserPlus size={14}/> Hozzárendelt csapattagok</span>{active.can_edit?<details className="group relative"><summary className="flex cursor-pointer list-none items-center justify-between rounded-xl border border-border bg-background px-3.5 py-3 text-sm font-semibold hover:border-primary/40"><span className="flex min-w-0 items-center gap-2"><Users size={16} className="shrink-0 text-primary"/><span className="truncate">{(active.assignee_ids||active.assignees?.map(item=>item.id)||[]).length?`${(active.assignee_ids||active.assignees?.map(item=>item.id)||[]).length} csapattag kiválasztva`:"Csapattagok kiválasztása"}</span></span><ChevronRight size={16} className="transition-transform group-open:rotate-90"/></summary><div className="absolute inset-x-0 top-[calc(100%+6px)] z-50 max-h-64 space-y-1 overflow-y-auto rounded-xl border border-border bg-surface/95 p-2 shadow-2xl backdrop-blur-xl">{teamMembers.map(member=>{const selected=(active.assignee_ids||active.assignees?.map(item=>item.id)||[]).includes(member.id);return <label key={member.id} className="flex cursor-pointer items-center gap-3 rounded-lg px-2.5 py-2 hover:bg-background"><input type="checkbox" checked={selected} onChange={()=>{const current=active.assignee_ids||active.assignees?.map(item=>item.id)||[];setActive({...active,assignee_ids:selected?current.filter(id=>id!==member.id):[...current,member.id]})}} className="h-4 w-4 accent-primary"/><span className="min-w-0 flex-1"><span className="block truncate text-xs font-semibold text-text">{member.name}</span><span className="block truncate text-[10px] text-muted-text">{member.email}</span></span><span className="rounded bg-primary/10 px-1.5 py-0.5 text-[9px] font-bold uppercase text-primary">{member.role}</span></label>})}{teamMembers.length===0&&<div className="px-2 py-4 text-center text-xs text-muted-text">Nincs elérhető csapattag.</div>}</div></details>:<div className="flex flex-wrap gap-2">{active.assignees?.length?active.assignees.map(member=><span key={member.id} className="flex items-center gap-1.5 rounded-full bg-primary/10 px-2.5 py-1 text-xs font-semibold text-primary"><Users size={12}/>{member.name}</span>):<span className="text-xs text-muted-text">Nincs hozzárendelt csapattag.</span>}</div>}<p className="mt-1.5 text-[10px] leading-4 text-muted-text">Ha van beállított e-mail-emlékeztető, minden kijelölt csapattag külön értesítést kap.</p></div>
