...
export function AdminCurrencyProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => { if (!token) return; fetch(`/api/admin/exchange-rates?base=EUR&quotes=${currencies.join(",")}`, { headers: { Authorization: `Bearer ${token}` } }).then(r => r.ok ? r.json() : null).then(data => { if (data) { setRates(data.rates || {}); setUpdatedAt(data.updated_at); localStorage.setItem("admin_exchange_rates_eur", JSON.stringify(data.rates || {})); } }).catch(() => {}); }, [token]);
