...
export function VisualIdeas({ settings }: { settings: SiteSettings }) {
  ...
  return (
    <section id="visual-ideas" data-nav-section="false" className="aero-section scroll-mt-20 px-6 py-20 md:py-28" aria-labelledby="visual-ideas-title">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 md:gap-4 lg:grid-cols-5">
          {items.map((item, index) => {
            ...
            return (
              <article
                key={item.id || index}
                className="aero-card group relative min-h-44 overflow-hidden rounded-2xl border border-border/80 p-5 transition-[transform,border-color,box-shadow] duration-300 hover:-translate-y-1 hover:border-primary/35 hover:shadow-lg"
              >
