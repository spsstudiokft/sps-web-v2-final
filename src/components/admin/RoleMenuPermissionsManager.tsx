...
const ROLES: Array<{ id: ConfigurableAdminRole; label: string; description: string }> = [
  ...
  { id: "video_editor", label: "Vágó", description: "Portfólió-, projekt- és leadási hozzáférés" },
  { id: "real_estate_agent", label: "Ingatlanügynök", description: "Ingatlanok, ügyfelek és projektek kezelése" },
  { id: "advertiser", label: "Hirdető", description: "Marketing- és hirdetési tartalmak kezelése" },
...
export function RoleMenuPermissionsManager() {
  ...
  return <div className="space-y-5">
    <div className="rounded-2xl border border-primary/20 bg-primary/5 p-4 flex gap-3"><ShieldCheck className="w-5 h-5 text-primary shrink-0 mt-0.5" /><div><p className="text-sm font-semibold text-text">Superadmin szerepkörjogosultságok</p><p className="text-xs text-muted-text mt-1 leading-relaxed">A Superadmin mindig minden menühöz hozzáfér. Az alábbi jelölők a többi szerepkör adminpanel-menüit és közvetlen oldal-hozzáférését szabályozzák.</p></div></div>
