...
export function EmailTemplateEditorModal({
  ...
  const [bodyFormat, setBodyFormat] = useState<"visual" | "html" | "text">("visual");
  ...
  const lastFocusedFieldRef = useRef<"subject" | "visual" | "html" | "text">("visual");
  ...
  // Keep the WYSIWYG canvas in sync without moving the cursor while the user types.
  useEffect(() => {
    if (bodyFormat !== "visual" || !visualEditorRef.current || document.activeElement === visualEditorRef.current) return;
    visualEditorRef.current.innerHTML = bodyHtml;
  }, [bodyFormat, bodyHtml]);

  const rememberVisualSelection = () => {
    const editor = visualEditorRef.current;
    const selection = window.getSelection();
    if (!editor || !selection?.rangeCount) return;
    const range = selection.getRangeAt(0);
    if (editor.contains(range.commonAncestorContainer)) visualSelectionRef.current = range.cloneRange();
  };

  const syncVisualHtml = () => setBodyHtml(visualEditorRef.current?.innerHTML || "");
  ...
  const restoreVisualSelection = () => {
    ...
    editor.focus();
    const selection = window.getSelection();
    if (selection && visualSelectionRef.current) {
  ...
  const handleInsertToken = (tokenStr: string) => {
    ...
    } else if (bodyFormat === "visual" && visualEditorRef.current) {
      restoreVisualSelection();
      const selection = window.getSelection();
      const range = selection?.rangeCount ? selection.getRangeAt(0) : null;
      if (range) {
        range.deleteContents();
        const tokenNode = document.createTextNode(tokenStr);
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div className="inline-flex rounded-lg p-0.5 bg-surface border border-border">
                        <button
                          type="button"
                          onClick={() => {
                            setBodyFormat("visual");
                            lastFocusedFieldRef.current = "visual";
                          }}
                    ...
                    <div className="overflow-hidden rounded-xl border border-border bg-slate-100 shadow-inner">
                      ...
                      <div className="max-h-[520px] overflow-y-auto bg-[#eef2f7] p-4 sm:p-7">
                        <div
                          ...
                          onInput={() => { rememberVisualSelection(); syncVisualHtml(); }}
                          onBlur={() => { rememberVisualSelection(); syncVisualHtml(); }}
                          className="mx-auto min-h-[360px] max-w-[600px] rounded-sm bg-white px-7 py-8 text-[16px] leading-7 text-slate-700 shadow-lg outline-none focus:ring-2 focus:ring-primary/40 sm:px-10"
                        />
                      </div>
                    </div>
                  ) : bodyFormat === "html" ? (
