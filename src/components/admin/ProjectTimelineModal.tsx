    ...
    <div className="fixed inset-0 z-[100] bg-black/70 backdrop-blur-sm p-3 sm:p-6 overflow-y-auto" role="dialog" aria-modal="true" aria-label={`${project.name} timeline`}>
      <div className="aero-frost-modal max-w-6xl mx-auto rounded-3xl border border-border bg-background shadow-2xl overflow-hidden">
        <header className="sticky top-0 z-20 px-5 sm:px-7 py-4 border-b border-border bg-background/90 backdrop-blur-xl flex items-center justify-between gap-4">
          <div><h2 className="text-xl font-bold text-text">{tUi("client.projects.timeline")}</h2><p className="text-sm text-muted-text">{project.name} · milestones and client updates</p></div>
        ...
        <div className="p-4 sm:p-6 grid lg:grid-cols-2 gap-6">
          <section className="space-y-4">
            <div className="rounded-2xl border border-border bg-surface p-4 space-y-3">
              ...
              <div className="grid sm:grid-cols-2 gap-3"><select className="px-3 py-2.5 rounded-xl border border-border bg-background text-text text-sm" value={milestoneForm.status} onChange={(e) => setMilestoneForm({ ...milestoneForm, status: e.target.value })}><option value="pending">{tUi("admin.team.status_pending")}</option><option value="in_progress">{tUi("client.projects.milestone_in_progress")}</option><option value="completed">{tUi("admin.projects.status_completed")}</option></select><Input type="datetime-local" value={milestoneForm.due_date} onChange={(e) => setMilestoneForm({ ...milestoneForm, due_date: e.target.value })} /></div>
            ...
            <div className="space-y-2">
              {milestones.map((item) => <div key={item.id} className="rounded-2xl border border-border bg-surface p-4 flex gap-3"><div className="pt-1">{item.status === "completed" ? <CheckCircle2 className="w-5 h-5 text-emerald-500" /> : item.status === "in_progress" ? <Clock3 className="w-5 h-5 text-amber-500" /> : <Circle className="w-5 h-5 text-muted-text" />}</div><div className="flex-1 min-w-0"><div className="font-semibold text-text">{item.title}</div>{item.description && <p className="text-xs text-muted-text mt-1">{item.description}</p>}<div className="flex flex-wrap items-center gap-2 mt-3"><select value={item.status} disabled={saving} onChange={(e) => changeMilestoneStatus(item, e.target.value)} className="text-xs px-2 py-1.5 rounded-lg bg-background border border-border text-text"><option value="pending">{tUi("admin.team.status_pending")}</option><option value="in_progress">{tUi("client.projects.milestone_in_progress")}</option><option value="completed">{tUi("admin.projects.status_completed")}</option></select>{item.client_notified_at && <span className="text-[11px] text-primary flex items-center gap-1"><Mail className="w-3 h-3" />Client notified</span>}</div></div><button type="button" onClick={() => remove("milestones", item.id)} className="p-2 text-muted-text hover:text-red-500"><Trash2 className="w-4 h-4" /></button></div>)}
