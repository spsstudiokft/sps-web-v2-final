...
export function BudgetEntryModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isNaN(numAmount) || numAmount < 0) {
      setErrorMessage("Az összeg nem lehet negatív.");
      return;
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs overflow-y-auto">
      <div className="bg-surface rounded-2xl border border-border shadow-2xl w-full max-w-lg overflow-hidden my-8 animate-in fade-in zoom-in-95 duration-200">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="col-span-2">
              <label className="block text-xs font-semibold text-text mb-1.5">
                {tUi("admin.budget.modal.amount")} <span className="font-normal text-muted-text">(opcionális)</span>
              </label>
              <div className="relative">
                <input
                  ...
                  step="any"
                  min="0"
                  placeholder="Üresen: ingyenes / természetbeni"
                  value={amount}
          ...
          <p className="-mt-3 text-[11px] leading-relaxed text-muted-text">Ha az összeget üresen hagyod, a tétel 0 értékkel, ingyenes vagy természetbeni szolgáltatásként kerül mentésre.</p>
