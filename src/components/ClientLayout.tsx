...
import { Building2, CircleHelp, Crown, FileText, FolderKanban, Gift, LayoutDashboard, LogOut, Menu, Settings, UserRound, X } from "lucide-react";
...
export default function ClientLayout() {
  ...
  const [spsRawAccess, setSpsRawAccess] = useState(false);
  useEffect(() => { setMobileMenuOpen(false); }, [location.pathname]);
  useEffect(() => { fetchApi("/api/client/sps-raw/access").then(async response => { if (!response.ok) return; const access = await response.json(); setSpsRawAccess(Boolean(access.enabled && access.vip)); }).catch(() => {}); }, [fetchApi]);
  ...
  const navigation: NavigationItem[] = [
    ...
    ...(spsRawAccess ? [{ to: "/client/sps-raw", label: "SPS RAW", icon: Crown, active: (path: string) => path.startsWith("/client/sps-raw") }] : []),
  ...
  const navigationContent = <div className="aero-client-sidebar flex h-full flex-col bg-background text-text">
    ...
    <div className="border-t border-border p-3"><div className="mb-2 flex items-center justify-between gap-2"><Link to="/client/settings" className="min-w-0 flex flex-1 items-center gap-3 rounded-xl p-2.5 hover:bg-surface"><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-primary/10 text-primary font-bold">{displayName.slice(0, 1).toUpperCase()}</div><div className="min-w-0"><p className="truncate text-sm font-semibold text-text">{displayName}</p><p className="truncate text-[11px] text-muted-text">{user?.email}</p></div></Link><PortalNotificationBell portal="client" compact /></div><Button variant="secondary" size="sm" onClick={handleLogout} className="w-full justify-start gap-2.5"><LogOut className="h-4 w-4" />{tUi("client.nav.logout") || "Kijelentkezés"}</Button></div>
  </div>;
  ...
  return <div className="aero-workspace aero-client-shell min-h-screen bg-background lg:flex">
    ...
    <header className="sticky top-0 z-30 flex min-h-16 items-center justify-between border-b border-border bg-background/90 px-4 backdrop-blur lg:hidden"><button type="button" onClick={() => setMobileMenuOpen(true)} className="inline-flex min-h-11 min-w-11 items-center justify-center rounded-xl border border-border bg-surface text-text" aria-label="Navigáció megnyitása" aria-expanded={mobileMenuOpen}><Menu className="h-5 w-5" /></button><p className="truncate px-3 text-sm font-bold text-text">{tUi("client.nav.portal_title") || "Ügyfélportál"}</p><div className="flex items-center gap-2"><PortalNotificationBell portal="client" compact openUp={false} /><Link to="/client/settings" className="flex h-10 w-10 items-center justify-center rounded-full bg-primary/10 text-sm font-bold text-primary" aria-label="Fiókbeállítások">{displayName.slice(0, 1).toUpperCase()}</Link></div></header>
